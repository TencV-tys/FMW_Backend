    ...
    try { 
      const stats = await db('notifications')
        ...
        .select(
          ...
          db.raw('SUM(CASE WHEN type = "resolution_request_submitted" THEN 1 ELSE 0 END) as resolution_request_submitted')
        )
        .first();
      ...
      res.json({
        ...
        stats: {
          ...
          resolution_request_submitted: parseInt(stats.resolution_request_submitted) || 0
        }
      });
    } catch (error) {
